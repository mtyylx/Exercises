...
public class Basic_Grammar {
    ...
    /** <中括号包裹特定字符集>来匹配特定字符集的内容 */
    static void characterMatcher_Test() {
        // 如果一般的内容匹配表达式（\w, \s, \d）不能满足你的需求，只需要用中括号包裹你想匹配的字符集即可。
        // [aeiou]: 匹配元音字母的任意一个。
        // [0-9]: 等效于\d，匹配任意数字。
        // [0-9A-Za-Z_]: 类似于\w，匹配任意字符/数字和下划线。
        find("[@#%]+", "@##%@##%_#");     // 匹配任意连续出现的@,#,%这三个字符之一的字符串，匹配结果为"@##%@##%"和"#".
        find("[0-8]la", "Hey2la9la");     // 匹配任意以0至8字符开头紧接着la的字符串，匹配结果为"2la".
    }

...
public class Basic_Insertion_Sort extends SortMethod {
    ...
    /** 插入排序的思想：外循环扫描<未排序区间>，内循环扫描<已排序区间>并对相邻元素进行平移，最后将未排序元素插入已排序区间的合适位置。*/
    ...
    //  sorted | unsorted
    // ← 内循环   外循环 →
    // 插入排序的核心任务，就是把未排序区域的第一个元素curr插入到已排序区域的合适位置去。
    // 这个“合适位置”的定义是：curr大于合适位置左侧相邻元素，小于等于合适位置右侧相邻元素。
    // 易错点1：内循环条件的设置。应该比较已排序元素（j指向的元素）和第一个未排序元素（curr缓存值）的大小关系。
    //         已排序区域一定满足 a[j] <= a[j + 1]，因此不能靠比较相邻元素判断是否可以插入
    //         第一个未排序元素由于可能会被覆盖，因此也不能靠比较a[j] > a[i]来判断是否可以插入
    // 易错点2：需要缓存外循环索引对应的元素值，因为平移会导致该值暂时被覆盖。
    // 易错点3：需要提前定义内循环指针，因为内循环结束之后，还需要该指针将被覆盖的元素恢复。
    static void InsertionSort1(int[] a) {
        for (int i = 1; i < a.length; i++) {                /** 外循环：正序扫描整个数组。<i指向未排序区域的第一个元素> */
            int j;                                          // 需要提前定义内循环指针，这样退出内循环后才可以将当前元素值复原。
            int curr = a[i];                                // 需要缓存当前元素值，因为内循环将会覆盖当前元素
            for (j = i - 1; j >= 0 && a[j] > curr; j--)     /** 内循环：逆序扫描已排序区域。<j初始指向已排序区域的最后一个元素> */

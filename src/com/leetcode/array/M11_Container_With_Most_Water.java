...
public class M11_Container_With_Most_Water {
    public static void main(String[] args) {
        ...
        System.out.println(maxContainer2(new int[] {5, 3, 6, 6, 8, 6, 12, 8}));
    }

    /** 解法2：双指针首尾相向扫描。优化解空间扫描。Time - o(n) */
    ...
    // * 如果下一个高度增加，但矩形高度依然由指针B决定，因此高度不变，但因为宽度收缩，所以整体面积会减小。
    // * 如果下一个高度减小至比指针B高度还小，矩形高度将会由这个更小的高度决定，再加上宽度收缩，因此整体面积同样会减小，而且减小的更多。
    // 因此我们就有理由不去移动指针A了。我们找到了一种可靠的变化趋势，可以用于精简解空间的搜索，也就不再是穷举法了。

    static int maxContainer2(int[] a) {
        int left = 0;
        int right = a.length - 1;
        int max = Math.max(a[left], a[right]) * a.length;
        while (left < right) {
            if (a[left] > a[right]) right--;
            else left++;
            max = Math.max(max, Math.max(a[left], a[right]) * (right - left));
        }
        return max;
    }
